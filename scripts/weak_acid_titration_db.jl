...
sol = exp.(solve(prob, opt; variable_space = Val(:log), verbose = 5, abstol = 1.0e-10, reltol = 1.0e-10))
sol = solve(prob, opt; variable_space = Val(:log), verbose = 5, abstol = 1.0e-10, reltol = 1.0e-10)
...
function numpH(p, Vb)
    ...
    prob = EquilibriumProblem(SM.A, μ, n0(p), ub = ub(p), p = p)
    sol = solve(prob, opt; variable_space = Val(:log), verbose = 0, abstol = 1.0e-12, reltol = 1.0e-12)
    pp = NamedTuple(p)
    Va, Vb = pp.Va, pp.Vb
    V = Va + Vb
