...
dict_all_species = Dict(symbol(s) => s for s in build_species_from_database(df_union))
...
dict_hydrates = Dict(symbol(s) => s for s in build_species_from_database(df_hydrates_clinker))
...
species = build_species_from_database(df_substances)
dict_species = Dict(symbol(s) => s for s in species)
dict_reactions = Dict(symbol(r) => r for r in build_reactions_from_database(df_reactions, species))
