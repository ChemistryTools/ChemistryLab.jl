...
```julia
dict_species = Dict(symbol(s) => s for s in build_species(df_union))
```

!!! warning "Calculation of Molar Mass"
    The function `build_species` is rather time costly due to the construction of `ThermoFunction`s so that it can be long on large databases.
...
```@setup example1
    ...
    dict_species = Dict(symbol(s) => s for s in build_species(df_union)) #hide
