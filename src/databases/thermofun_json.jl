...
# """
#     read_thermofun_substances(substances::JSON3.Array) -> DataFrame
#     read_thermofun_substances(filename::AbstractString) -> DataFrame
...
# """
#     read_thermofun_reactions(reactions::JSON3.Array) -> DataFrame
#     read_thermofun_reactions(filename::AbstractString) -> DataFrame
...
# """
#     read_thermofun_json(filename::AbstractString) -> Tuple{DataFrame, DataFrame, DataFrame}
...
# #     df_substances = read_thermofun_substances(data.substances)
# #     df_reactions = read_thermofun_reactions(data.reactions)
...
function extract_value(row, field::Symbol; verbose=false, default_unit=u"1")
...
function complete_species_with_thermo_model!(species, row; verbose=false)
    ...
    for (k,v) in values0
        if !haskey(species, k) && !ismissing(v)
            species[k] = ThermoFunction(v; ref=[:T => Tref, :P => Pref])
        end
    end
    return species
end
...
function complete_reaction_with_thermo_model!(reaction, row; verbose=false)
    ...
    for (k,v) in values0
        if !ismissing(v)
            ...
            if !haskey(reaction, k) reaction[k] = ThermoFunction(v; ref=[:T => Tref, :P => Pref]) end
        end
    end
    return reaction
end
