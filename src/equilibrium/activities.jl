...
# Extract the primal Float64 value from a possibly-Dual number for safe branching.
_primal(x::Real) = x
_primal(x::ForwardDiff.Dual) = ForwardDiff.value(x)
...
function activity_model(cs::ChemicalSystem, model::HKFActivityModel)
    ...
    function lna(n::AbstractVector, p)
        ...
        å_eff    = sum_mz2 > ϵ ? sum_mz2a / sum_mz2 : model.å_default
