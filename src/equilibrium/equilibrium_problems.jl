@kwdef struct EquilibriumProblem{μType<:Function,T<:Number, P, S}
    ...
    function EquilibriumProblem(A, μ, u0; p=SciMLBase.NullParameters(),
        ...
        ϵ = 1.e-16
        return new{typeof(μ), eltype(u0), typeof(p), eltype(indep_components)}(A*u0, A, μ, u0, p, max.(lb, ϵ), max.(ub, ϵ), indep_components, dep_components)
    end
end
...
SciMLBase.solve(ep::EquilibriumProblem, vartype, solver; kwargs...) = solve(OptimizationProblem(ep, vartype), solver; kwargs...)
